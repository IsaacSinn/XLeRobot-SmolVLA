...
class ElrobotLeader(Teleoperator):
    """
    Elrobot eight-motor Feetech leader arm (joint_1..joint_7 + gripper) for teleoperation.
    ...
    def calibrate(self) -> None:
        ...
        # joint_5 and joint_7 are near-full-turn (±170°)
    ...
    def get_action(self) -> dict[str, float]:
        """Read current joint positions and clip to URDF limits (degrees mode only)."""
        ...
        if self.config.use_degrees and self.config.joint_limits:
            for joint, (lo, hi) in self.config.joint_limits.items():
                key = f"{joint}.pos"
                if key in action:
                    action[key] = float(np.clip(action[key], lo, hi))
